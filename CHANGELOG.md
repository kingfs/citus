...
* Fixes ALTER TABLE IF EXISTS SET SCHEMA bug with non-existing tables

* Fixes CREATE INDEX CONCURRENTLY crash with local execution

* Fixes CREATE INDEX CONCURRENTLY with no index name crash with postgres tables
...
* Fixes a crash because of overflow in partition id
...
* Forces aliases in deparsing queries with anonymous column references
